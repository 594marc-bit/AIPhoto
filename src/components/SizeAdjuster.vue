...
const updateScalePercent = (value: number | null) => {
...
const updateMaxSize = (value: number | null) => {
  if (value === null) return;
  maxSize.value = value;
  updateScalePercent(scalePercent.value);
};

const updateFixedWidth = (value: number | null) => {
  fixedWidth.value = value ?? undefined;
  if (sizeMode.value === 'fixed') {
    emit('update:outputWidth', fixedWidth.value);
  }
};

const updateFixedHeight = (value: number | null) => {
  fixedHeight.value = value ?? undefined;
  if (sizeMode.value === 'fixed') {
    emit('update:outputHeight', fixedHeight.value);
  }
};
...
const updateQuality = (value: number | null) => {
  if (value === null) return;
  quality.value = value;
