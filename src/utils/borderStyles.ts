 ...
 * @param imageWidth - Optional image width to calculate appropriate bottom border height
 * @param imageHeight - Optional image height to calculate appropriate bottom border height
 */
export function getDefaultBorderStyle(imageWidth?: number, imageHeight?: number): BorderStyle {
  // Calculate bottomHeightPercent based on image orientation
  // Landscape (width >= height): 1/9 of image height ≈ 11.11%
  // Portrait (width < height): 1/14 of image height ≈ 7.14%
  let bottomHeightPercent = 5; // Default fallback
  if (imageWidth && imageHeight) {
    if (imageWidth >= imageHeight) {
      // Landscape: use 1/9
      bottomHeightPercent = 100 / 9;
    } else {
      // Portrait: use 1/14
      bottomHeightPercent = 100 / 14;
    }
  }
