...
function parseExifFromView(view: DataView): ExifData | null {
  ...
  while (reader.offset < reader.length) {
    ...
    // Get segment length
    const segmentLength = view.getUint16(reader.offset);
    reader.offset += 2;
...
function parseExifData(tags: Record<string, unknown>): ExifData | null {
  ...
  if (tags.Make) data.make = String(tags.Make).trim();
  if (tags.Model) data.model = String(tags.Model).trim();

  // Date/time
  if (tags.DateTimeOriginal) {
    data.dateTime = formatDate(String(tags.DateTimeOriginal));
  } else if (tags.DateTime) {
    data.dateTime = formatDate(String(tags.DateTime));
  }
  ...
  if (tags.PhotographicSensitivity !== undefined) {
    data.iso = `ISO ${tags.PhotographicSensitivity}`;
    ...
    data.iso = `ISO ${tags.RecommendedExposureIndex}`;
  ...
  if (tags.LensModel) {
    data.lensModel = String(tags.LensModel).trim();
  }
