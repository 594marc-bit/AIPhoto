...
export class CanvasRenderer {
  ...
  async render(options: RenderOptions): Promise<RenderResult> {
    ...
    if (borderStyle.blur) {
      await this.drawBlurBackground(img, width, height, borderStyle);
    } else {
    ...
    if (logoConfig && borderStyle.showLogo) {
      await this.drawLogo(logoConfig, borderStyle, width, height, leftWidth, rightWidth, topWidth, bottomHeight, originalWidth);
    }

    // Convert to blob
    const blob = await this.toBlob(options.quality);
   ...
   */
  private async drawBlurBackground(
    img: HTMLImageElement,
    canvasWidth: number,
    canvasHeight: number,
    borderStyle: BorderStyle,
  ): Promise<void> {
    const padding = borderStyle.padding || 20;

    // Create a temporary canvas for blur effect
    const tempCanvas = document.createElement('canvas');
    tempCanvas.width = canvasWidth;
    tempCanvas.height = canvasHeight;
    const tempCtx = tempCanvas.getContext('2d') as CanvasRenderingContext2D | null;
    if (!tempCtx) return;
  ...
  private drawExifInfo(
    ...
    const padding = borderStyle.padding || 10;
    ...
    const horizontalPadding = padding || 15;
    ...
    let textY = bottomBorderCenter - textBlockHeight / 2 + (canvasHeight * (textStyle.textOffsetY ?? 0)) / 100;
  ...
  private async drawLogo(
    ...
    try {
      ...
      let x = centerX + (canvasWidth * (logoConfig.offsetX ?? 0)) / 100;
      ...
      if (logoConfig.position === 'top') {
        y = topWidth + (canvasHeight * (logoConfig.offsetY ?? 0)) / 100;
      } else {
        // Position from bottom border center, offset by percentage of canvas height
        y = bottomBorderCenter - logoHeight / 2 + (canvasHeight * (logoConfig.offsetY ?? 0)) / 100;
      }
